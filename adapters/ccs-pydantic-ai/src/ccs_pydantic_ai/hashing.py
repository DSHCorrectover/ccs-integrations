...
the canonicalization so that :mod:`ccs_pydantic_ai.signer` and
:mod:`ccs_pydantic_ai.receipt_builder` never diverge from the canonical form used
by ``ccs-verifier``.
"""
...
def _validate_safe_integers(data: Any) -> None:
    ...
    Mirrors the check performed by
    ``ccs_verifier.ccs_verifier_l1.canonical_json``. Booleans (``int``
    subclasses) are excluded.
    """
