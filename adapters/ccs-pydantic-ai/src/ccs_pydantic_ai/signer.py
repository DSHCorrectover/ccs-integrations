...
* :class:`InProcessSigner` — derives the private key deterministically from a
  ...
  the agent process; receipts are byte-reproducible across runs.
* :class:`SidecarSigner` — keeps the private key out of the agent process. It
...
:class:`~ccs_pydantic_ai.receipt_builder.ReceiptBuilder` can sign either L1 or
...
def derive_in_process_key(seed: bytes) -> Ed25519PrivateKey:
    ...
    (``SHA-256(b'ccs-verifier/in-process-test/v1')``).
    """
...
class SidecarSigner:
    ...
    The private key never enters the agent process. Signing is delegated to an
...
def build_signer(config: Any) -> CCSSigner:
    """Construct the appropriate signer for a :class:`~ccs_pydantic_ai.config.CCSConfig`.
