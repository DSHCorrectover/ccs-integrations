...
def validate_l1_structure(receipt: dict[str, Any]) -> tuple[bool, str]:
    ...
    # public_key_fingerprint must be 16 hex chars
...
def verify_l1_receipt(
    ...
    *,
    ...
    Args:
        receipt: The L1 receipt dict.
        check_expiry: When ``True``, also reject receipts whose ``expires_at``
            is in the past (allowing for ``max_clock_skew``).
        now: Override current time for testing (epoch seconds).
